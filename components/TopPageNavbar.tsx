...
const TopPageNavbar: FC<TopPageNavbarprop> = async ({ title }) => {
  ...
  return (
    <div className="my-0 mx-auto relative flex items-center w-full max-w-[1040px] z-10">
      {/* ARROW ICON  */}
      <SlideBarOuterButton />
      <h1 className="text-[#393a4f] font-bold text-2xl hidden sm:block  ">
        {title} Page
      </h1>
      ...
      <div className="flex ml-auto items-center ">
        <div className="px-[6px] flex items-center py-2 grow-0 shrink-0 text-[#4a4a4a] relative ">
          <Link
            href={"https://www.linkedin.com/in/taqui-imam-88a7b325a/"}
            className="relative flex justify-center items-center w-[38px] h-[38px] rounded-sm decoration-transparent "
          >
          ...
          <div className="relative max-h-[38px]">
            {session?.user?.image && (
              <Image
                ...
                height={38}
                width={38}
                className="rounded-full max-h-[38px]"
              />
            )}
          </div>
        </div>
