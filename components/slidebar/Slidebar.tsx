...
async function Slidebar() {
  ...
  return (
    <div
      id="slidebar"
      className="absolute top-0 left-0 h-full w-[280px] bg-white border-r  border-r-borderE3 shadow-md z-30 transition-transform myslidebar"
    >
      {/* top section -->  */}
        ...
        {session?.user && (
          <div>
            <Image
              ...
              loading="eager"
              priority
              className="block h-[70px] w-[70px] max-w-full mb-4 rounded-full"
            />
            <div className="">
              <span className="block font-bold text-xl text-[#393a4f] font-sans ">
                {session.user.name}
              </span>
              <span className=" block text-sm text-[#a2a5b9] font-serif ">
                Public
              </span>
            </div>
          </div>
