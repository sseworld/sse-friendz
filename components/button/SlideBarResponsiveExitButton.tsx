...
export const SlideBarInnerButton = () => {
  ...
  return (
    <button onClick={handleResponsiveButton}>
      {" "}
      <AiOutlineArrowLeft className="absolute top-[10px] right-3 block items-center justify-center h-8 w-8 outline-none border-none md:hidden " />
    </button>
  );
};
export const SlideBarOuterButton = () => {
  ...
  return (
    <div className="mr-4 cursor-pointer block" onClick={hideSlidebar}>
      {!open && (
        <span className="h-fit w-fit hidden md:block" onClick={hideSlidebar}>
          <IoIosArrowBack className="h-7 w-7 text-[#3180e1] " />
        </span>
      )}
      {open && (
        <span className="h-fit w-fit hidden md:block" onClick={hideSlidebar}>
          <RiMenu2Fill className="h-7 w-7 text-[#3180e1] " />{" "}
        </span>
      )}
      <span
